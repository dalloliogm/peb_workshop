...
We will take a "messy" gene expression table, and use the tidyr library to restructure it in a format that is better suited for data analysis. We will also group the data and calculate summaries using the dplyr library, and plot it using ggplot2.
...
A dataset can be encoded in a "wide" format (more columns and less rows), or in a "long" format (minimum number of columns and more rows). While the wide format may be sometimes more readable for the human eye, the long format is better suited for data analysis. 

Our peb data frame is in a "wide" format, as it contains one column for every individual. However, all the functions used in the rest of the workshop require the dataset to be in a long format. Let's convert it using the gather() function from tidyr:
...
Note how the new format has only four columns. Many recent R libraries are designed for datasets in the long format. A common beginner mistake is to apply these libraries to datasets in the wide format, while instead you should re-format your dataset first. 
...
- *mutate* is used to add columns to a data frame. For example we can use it to check if a given gene is over or under expressed in an individual, compared to the other genes:
